...
CREATE TABLE section (
    ...
    year_level_id INTEGER REFERENCES year_level(year_level_id)
);
...
CREATE TABLE storage_history (
    ...
    invitem_id INTEGER REFERENCES inventory_item(invitem_id),
    action VARCHAR(50),
    quantity_before INTEGER,
    quantity_after INTEGER,
    change INTEGER,
    dispensed_to_student_id VARCHAR(30) REFERENCES student(stud_id),
    dispensed_date DATE,
    details TEXT,
    created_at TIMESTAMP
);
