...
class MedicationRecordsPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.table.setColumnCount(9)  # 9 columns: Student ID, Name, Medication, Dosage, Frequency, Start, End, Notes, Actions
    ...
    def load_data(self):
        ...
        try:
            ...
            cur = conn.cursor()
            # Concatenate last, first, (optional) middle name for display
            cur.execute("""
            ...
            for row_idx, row in enumerate(rows):
                ...
                self.table.setCellWidget(row_idx, 8, btn_widget)  # index 8 for 9th column
            cur.close()
            conn.close()
    ...
    def edit_medication(self, row):
        ...
        # skip stud_fullname for editing
        edit_page.fill_fields(row[1], row[3], row[4], row[5], row[6], row[7], row[8], med_id=row[0])
